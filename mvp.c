...
int render_ship(ship *ship, uch in_wing_pos) {
    ship_type type;
    uch i, casted_type;
    
    i = in_wing_pos;
    type = ship->type;
    casted_type = (uch) type;
...
uch read_action() {
    ...
    for (i = 0; i < chooserMax; ++i){
        chooserPos[i][0] = gl_options_pos[i][0];
        chooserPos[i][1] = gl_options_pos[i][1];
    }
    choose();
    printf("action %c", chooserCursor);
    return chooserCursor;
}

char read_ship_i(wing *wing, uch side) {
    int a;
    uch i;
    chooserMax = wing->size - 1;
    for (i = 0; i < wing->size; i++) {
            chooserPos[i][0] = SHIP_OFFSET;
            chooserPos[i][1] = gl_shipPos[i + side];
    }
    choose();
    a = chooserCursor;
    printf("target %d", a);
...
void draw_fight_options() {
    WRITE_GLYPH(1, chooserPos[0][0], chooserPos[0][1]);
    WRITE_GLYPH(2, chooserPos[1][0], chooserPos[1][1]);
    WRITE_GLYPH(3, chooserPos[2][0], chooserPos[2][1]);
}
