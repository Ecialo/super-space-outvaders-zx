...
void add_ship(wing *wing, ship_type ship_type) {
    uch slot;
...
void fill_wing_with_rand_ships(wing *wing, uch tier1, uch tier2, uch mods) {
    ...
    uch i;
    uch mod;
    for (i = 0; i < tier1 + tier2; i++) {
        t = (ship_type) (rand() % 4);
        add_ship(wing, t);
    }
    for (i = 0; i < tier2; i++) {
        promote_ship(get_ship(wing, i));
    }
    while (mods > 0)
    {
